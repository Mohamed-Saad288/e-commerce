                    ...
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h4 class="card-title">{{ __('organizations.option_items') }}</h4>
                            <a href="{{ route('organization.option_items.create') }}" class="btn btn-primary">
                                {{ __('organizations.add_option_item') }}
                            </a>
                    </div>
                    <div class="card-body">
                                ...
                                @if (count($option_items) > 0)
                                    @foreach ($option_items as $option_item)
                                        <tr>
                                            ...
                                            <td>{{ $option_item->option?->name ?? "-"}}</td>
                                            <td>
                                                    <a href="{{ route('organization.option_items.edit', $option_item->id) }}"
                                                       class="btn btn-sm btn-success">
                                                        <i class='fe fe-edit fa-2x'></i>
                                                    </a>

                                                    <button class="btn btn-sm btn-danger delete-option_item"
...
@section('after_script')
    ...
    <script>
            ...
            $(document).on('click', '.delete-option_item', function (e) {
                e.preventDefault();
                let option_itemId = $(this).data('id');
                let deleteUrl = "{{ route('organization.option_items.destroy', ':id') }}".replace(':id', option_itemId);
