        ...
        <div class="col-md-12">
            <div class="form-group">
                ...
                <textarea name="{{ $locale }}[question]" id="question_{{ $locale }}"
                          ...
                          rows="3">{{ old("$locale.question", isset($category) ? $category->translate($locale)->question : '') }}</textarea>
        ...
        <div class="col-md-12">
            <div class="form-group">
                ...
                <textarea name="{{ $locale }}[answer]" id="answer_{{ $locale }}"
                          ...
                          rows="5">{{ old("$locale.answer", isset($category) ? $category->translate($locale)->answer : '') }}</textarea>
                @error("$locale.answer")
                <small class="text-danger">{{ $message }}</small>
                @enderror
            </div>
