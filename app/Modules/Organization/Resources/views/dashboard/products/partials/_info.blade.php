        ...
        <div class="row">
            <div class="col-md-6">
                <table class="table table-sm table-borderless mb-0">
                    <tr><th class="text-muted w-40">{{ __('messages.name') }}:</th><td>{{ $product->name }}</td></tr>
                    <tr><th class="text-muted">{{ __('messages.slug') }}:</th><td><code>{{ $product->slug }}</code></td></tr>
                    <tr><th class="text-muted">{{ __('messages.sku') }}:</th><td>{{ $product->sku ?? '-' }}</td></tr>
                    <tr><th class="text-muted">{{ __('messages.barcode') }}:</th><td>{{ $product->barcode ?? '-' }}</td></tr>
                    <tr><th class="text-muted">{{ __('organizations.category') }}:</th>
                        <td><span class="badge badge-soft-info">{{ $product->category?->name ?? '-' }}</span></td>
                    </tr>
                </table>
            </div>
            <div class="col-md-6">
                <table class="table table-sm table-borderless mb-0">
                    <tr><th class="text-muted w-40">{{ __('organizations.brand') }}:</th>
                        <td><span class="badge badge-soft-success">{{ $product->brand?->name ?? '-' }}</span></td>
                    </tr>
                    ...
                    <tr><th class="text-muted">{{ __('organizations.stock') }}:</th>
                        <td>{!! $product->stock_quantity > 10
                    ...
                    <tr><th class="text-muted">{{ __('messages.type') }}:</th>
                        <td>
                            @switch($product->type)
                    ...
                    <tr><th class="text-muted">{{ __('messages.requires_shipping') }}:</th>
                        <td>{!! $product->requires_shipping
