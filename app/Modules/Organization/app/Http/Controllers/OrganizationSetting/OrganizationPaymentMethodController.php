        ...
        try {
            ...
            if (! $organization) {
                return response()->json([
                    'status' => 'error',
                    'message' => __('organizations.Organization not found.'),
                ], 404);
            }
            ...
            $orgPaymentMethod = OrganizationPaymentMethod::query()->firstOrNew([
            ...
            $message = $orgPaymentMethod->is_active ? __('organizations.Payment method activated successfully.')
                : __('organizations.Payment method deactivated successfully.');
            ...
            return response()->json([
                'status' => 'error',
                'message' => __('organizations.payment_method_not_found.'),
            ], 404);
        } catch (Exception $e) {
            ...
            return response()->json([
                'status' => 'error',
                'message' => __(
                    'organizations.An_error_occurred_while_updating_payment_method. Please_try_again_later.'
                ),
            ], 500);
        }
    }
